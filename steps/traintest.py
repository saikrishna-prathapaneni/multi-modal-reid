...
def train(audio_model, image_model, train_loader, test_loader, args):
    ...
    while True:
        ...
        audio_model.train()
        image_model.train()
        for i, (image_input, audio_input, nframes, id) in enumerate(train_loader):
            ...
            loss = sampled_margin_rank_loss(image_output, audio_output,
                nframes, id, margin=args.margin, simtype=args.simtype)
...
def validate(audio_model, image_model, val_loader, args, visualise_id = 10):
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    batch_time = AverageMeter()
    audio_model = audio_model.to(device)
    image_model = image_model.to(device)
    ...
    with torch.no_grad():
        for i, (image_input, audio_input, nframes, id) in enumerate(val_loader):
        ...
        if args.recall_type =='topn':
            ...
            recalls =calc_recalls_reid(image_output, audio_output, nframes, id_output, val_loader, visualise_id = 10, simtype=args.simtype)
            A_r20 = recalls['A_r20']
            I_r20 = recalls['I_r20']
            A_r10 = recalls['A_r10']
            I_r10 = recalls['I_r10']
